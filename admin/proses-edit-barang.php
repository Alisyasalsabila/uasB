...
if ($uploadOk == 0) {
    ...
    if (move_uploaded_file($_FILES["foto_barang"]["tmp_name"], $target_file)) {
        ...
        $stmt = $db_koneksi->prepare("UPDATE tbl_barang SET foto_barang=?, nama_barang=?, harga_barang=?, stok_barang=?, deskripsi_barang=?, spesifikasi_barang=?, warna_barang=?, ukuran_barang=? WHERE id_barang=?");
        $stmt->bind_param("sssssssss", basename($_FILES["foto_barang"]["name"]), $nama_barang, $harga_barang, $stok_barang, $deskripsi_barang, $spesifikasi_barang, $warna_barang, $ukuran_barang, $id);
        $stmt->execute();
